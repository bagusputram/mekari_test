...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a>{{ trans('manage-setting.subdistrict.subdistrict') }}</a></li>
    </ol>
</section>
                    ...
                    <div class="tab-content tab-content-company">
                        <div class="tab-pane tab-main active" id="tab-company" role="tabpanel">
                            <div class="box-header">
                                <h3 class="box-title">{{ trans('manage-setting.subdistrict.subdistrict') }} {{ trans('manage-setting.setting') }}</h3>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-12 col-12">
                    <div class="box">
                        <div class="tab-content tab-content-company">
                            <div class="tab-pane tab-main active" id="tab-company" role="tabpanel">
                                <div class="box-header">
                                    <h3 class="box-title" id="title-form">{{ trans('app.add_new') }} {{ trans('manage-setting.subdistrict.subdistrict') }}</h3>
                                    <div class="box-tools pull-right">
                                        <button type="button" class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip" title="Collapse">
                                            <i class="fa fa-minus"></i></button>
                                    </div>
                                    <div class="box-body">
                                        <form action="{{ route('setting.subdistrict.store') }}" method="post" class="form-setting-subdistrict" id='theform'>
                                                    ...
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="col-md-5">
                                                            <select class="form-control select" name="city_id" id="city_id">
                                                                <option value="0">{{ trans('manage-setting.subdistrict.choose_province') }}</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="form-group row">
                                                        <label for="district_id" class="col-md-2 col-form-label">{{ trans('manage-setting.district.district') }}<span class="text-danger">*</span></label>
                                                        <div class="col-md-5">
                                                            <select class="form-control select" name="district_id" id="district_id">
                                                                <option value="0">{{ trans('manage-setting.subdistrict.choose_city') }}</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="form-group row">
                                                        <label for="name" class="col-md-2 col-form-label">{{ trans('manage-setting.subdistrict.name') }}<span class="text-danger">*</span></label>
                                                        <div class="col-md-5">
                                                            <input class="form-control-app" type="text" id="name" name="name" value="" required>
                                                        </div>
                                                    </div>
                                                    <div class="form-group row">
                                                        <label for="postcode" class="col-md-2 col-form-label">{{ trans('manage-setting.subdistrict.postcode') }}<span class="text-danger">*</span></label>
                                                        <div class="col-md-5">
                                                            <input class="form-control-app" type="text" id="postcode" name="postcode" value="" maxlength="5" required>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div class="tab-pane tab-main active" id="tab-company" role="tabpanel">
                            <div class="box-header with-border">
                                <div class="table-filter" style="display:inline-block">
                                    <a href="{{ route('setting.subdistrict.index') }}" class="{{ Request::get('status') == NULL ? 'text-green' : 'text-muted' }}">
                                        {{ __('app.all_data') }}
                                        {{-- <span class="badge badge-pill badge-info">{{ $subdistrict_count }}</span> --}}
                                    </a>
                                    &nbsp; | &nbsp;
                                    <a href="{{ route('setting.subdistrict.index') }}?status=trash" class="{{ Request::get('status') != NULL ? 'text-green' : 'text-muted' }}">
                                        {{ __('app.trash') }}
                            ...
                            <div class="box-body">
                                <table id="ajax-table" class="table table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            <th class="text-center">{{ trans('manage-setting.subdistrict.subdistrict') }} {{ trans('manage-setting.city.name') }}</th>
                                            <th class="text-center">{{ trans('manage-setting.subdistrict.postcode') }}</th>
...
@push('more-js')
    <script type="text/javascript">
        ...
        var ajax_table_url      = '/api/setting/subdistrict/alldata/' + trash;
        var ajax_table_headers  = {'X-CSRF-TOKEN': '{{csrf_token()}}' };
        var ajax_table_columns  = [
            {data: 'name', name: 'subdistricts.name'},
            {data: 'postcode', name: 'subdistricts.postcode'},
            {data: 'districtname', name: 'subdistricts.district_id'},
            {data: 'cityname', name: 'subdistricts.city_id'},
            {data: 'provincename', name: 'subdistricts.province_id'},
            {data: 'action', name: 'action', orderable: false, searchable: false}
        ];

        function execRemove(method, hashid) {
            $("#action-form").attr('action', '/setting/subdistrict/' + hashid);
            $("#action-form input[name=_method]").val(method);
            $("#action-form").submit();
        };

        function execEdit(method, hashid) {
            axios.get('/api/setting/subdistrict/singledata/' + hashid).then(response => {
                var data = response.data;
                {{--  Change Title Form  --}}
                $("#title-form").text("{{ trans('app.edit') }} {{ trans('manage-setting.subdistrict.subdistrict') }}")
                {{--  Change Form Action  --}}
                $("#theform").attr('action', '/setting/subdistrict/' + hashid + '/edit');
                {{--  Chang Form Method  --}}
                $("#theform input[name=_method]").val(method);
                {{--  Change Form Value  --}}
                $("#theform input[name=name]").val(data[0].name);
                $("#theform input[name=postcode]").val(data[0].postcode);
        ...
        function restore (hashid) {
            $("#action-form").attr('action', '/setting/subdistrict/' + hashid + "/restore");
