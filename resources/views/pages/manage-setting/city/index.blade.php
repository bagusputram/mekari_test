...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><a href="{{ route('setting.master-data.index')}}">{{ trans('manage-setting.manage_setting') }}</a></li>
        <li class="active"><a>{{ trans('manage-setting.city.city') }}</a></li>
    </ol>
</section>
                ...
                <div class="col-lg-12 col-12">
                    <div class="box">
                        <div class="tab-content tab-content-company">
                            <div class="tab-pane tab-main active" id="tab-company" role="tabpanel">
                                <div class="box-header">
                                    <h3 class="box-title" id="title-form">{{ trans('app.add_new') }} {{ trans('manage-setting.city.city') }}</h3>
                                    <div class="box-tools pull-right">
                                        <button type="button" class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip" title="Collapse">
                                            <i class="fa fa-minus"></i></button>
                                    </div>
                                    <div class="box-body">
                                        <form action="{{ route('setting.city.store') }}" method="post" class="form-setting-city" id='theform'>
                                                        ...
                                                        <div class="col-md-5">
                                                            <select class="form-control select" name="province_id" id="province_id">
                                                                ...
                                                                @foreach ($provinces as $province)
                                                                    <option class="select-font" value="{{$province->id}}">{{$province->name}}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                        ...
                        <div class="tab-pane tab-main active" id="tab-company" role="tabpanel">
                            <div class="box-header with-border">
                                <div class="table-filter" style="display:inline-block">
                                    <a href="{{ route('setting.city.index') }}" class="{{ Request::get('status') == NULL ? 'text-green' : 'text-muted' }}">
                                        {{ __('app.all_data') }}
                                        <span class="badge badge-pill badge-info">{{ $city_count }}</span>
                                    </a>
                                    &nbsp; | &nbsp;
                                    <a href="{{ route('setting.city.index') }}?status=trash" class="{{ Request::get('status') != NULL ? 'text-green' : 'text-muted' }}">
                                        {{ __('app.trash') }}
                            ...
                            <div class="box-body">
                                <table id="data-table" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $city->name}}</td>
                                            <td>{{ ($city->provinceName) ? $city->provinceName->name : trans('app.deleted')}}</td>
                                            <td class="text-center">
                                                @if( $city->deleted_at == null )
                                                    <button type="button" onclick="willEdit('{{ $city->hashid }}', 'PUT')" class="btn btn-primary btn-sm btn-edit" id="btn-edit-{{ $loop->iteration }}" title="{{ __('app.edit') }}">
                                                        <i class="fa fa-pencil" style="pointer-events:none;"></i>
                                                    </button>
                                                    <button type="button" onclick="willRemove('{{ $city->hashid }}', 'DELETE')" class="btn btn-danger btn-sm btn-delete" id="btn-delete-{{ $loop->iteration }}" title="{{ __('button.remove') }}">
                                                        <i class="fa fa-trash" style="pointer-events:none;"></i>
                                                    </button>
                                                @else
                                                    @if($city->deleted_at != null)
                                                        <button type="submit" class="btn btn-success btn-sm btn-restore" onclick="restore('{{ $city->hashid }}')" title="{{ __('button.restore') }}">
                                                            <i class="fa fa-undo" style="pointer-events:none;"></i>
                                                        </button>
                                                        <button type="button" onclick="willRemove('{{ $city->hashid }}', 'DESTROY')" class="btn btn-danger btn-sm btn-delete" id="btn-delete-{{ $loop->iteration }}" title="{{ __('button.remove') }}">
                                                            <i class="fa fa-trash" style="pointer-events:none;"></i>
                                                        </button>
                                                    @endif
                                                @endif
...
@push('more-js')
    ...
    <script type="text/javascript">
        ...
        function execRemove(method, hashid) {
            $("#action-form").attr('action', '/setting/city/' + hashid);
            $("#action-form input[name=_method]").val(method);
            $("#action-form").submit();
        };

        function execEdit(method, hashid) {
            axios.get('/api/setting/city/' + hashid).then(response => {
                var data = response.data;
                {{--  Change Title Form  --}}
                $("#title-form").text("{{ trans('app.edit') }} {{ trans('manage-setting.city.city') }}")
                {{--  Change Form Action  --}}
                $("#theform").attr('action', '/setting/city/' + hashid + '/edit');
                {{--  Chang Form Method  --}}
        ...
        function restore (hashid) {
            $("#action-form").attr('action', '/setting/city/' + hashid + "/restore");
